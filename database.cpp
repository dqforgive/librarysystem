...
void database::quitLogin()
{
    ...
    loginbutton->setStyleSheet("color:rgb(0,0,255)");
    registerbutton = new QPushButton("注册");
    registerbutton->setStyleSheet("color:rgb(0,0,255)");
    loginbutton->setFlat(true);
    registerbutton->setFlat(true);
...
void database::Register()
{
    ...
    if(nameLine3->text().isEmpty()||passwordLine1->text().isEmpty()||passwordLine2->text().isEmpty()){
        QMessageBox::critical(NULL, "出错了", "您的信息填写不完整",
                              QMessageBox::Yes);
        return;
    }
    ...
    if(passwordLine1->text().compare(passwordLine2->text())!=0){
        QMessageBox::critical(NULL, "出错了", "两次密码输入不一致！",
                              QMessageBox::Yes);
        return;
    }
    ...
    if(query.next()){
        QMessageBox::critical(NULL, "出错了", "该学号已被注册",
                              QMessageBox::Yes);
        return;
    }
    ...
    if(query.isActive()){
        QMessageBox::about(0,"恭喜您","注册成功！");
    }
    else{
        QMessageBox::critical(NULL, "出错了", "注册失败！",
                              QMessageBox::Yes);
        return;
    }
...
void database::userLogin()
{
    ...
    if(query2.next()){
        ...
        if(QString::compare(sel_password,password1->text())!=0){
            QMessageBox::critical(NULL, "出错了", "密码错误",
                                  QMessageBox::Yes);
            return;
        }
    }
    //如果数据库中没有此学号，给出警告
        ...
        QMessageBox::critical(NULL, "出错了", "该用户名不存在",
                              QMessageBox::Yes);
        return;
    }
...
void database::managerLogin()
{
    ...
    query2.exec("select password from manager where username = '"+username2->text()+"'");
    if(!query2.isActive()){
        return;
    }
...
void database::searchBook()
{
    ...
    if(nameLine->text().isEmpty()&&publishLine->text().isEmpty()&&authorLine->text().isEmpty()
        ...
        QMessageBox::critical(0,QObject::tr("出错了"),"您尚未填写任何信息");
        return;
    }
    ...
    //检查日期和价格的填写是否正确
            ...
            (!priceLine->text().isEmpty()||priceLine1->text().isEmpty()||
             priceLine->text().isEmpty()||!priceLine1->text().isEmpty())){
        QMessageBox::critical(0,QObject::tr("出错了"),"您的日期和价格范围有误");
        return;
    }
    else if((dateBox->currentText().compare("    ")==0&&dateBox->currentText().compare("    ")!=0)||
            (dateBox->currentText().compare("    ")!=0&&dateBox->currentText().compare("    ")==0)){
        QMessageBox::critical(0,QObject::tr("出错了"),"您的日期范围有误");
        return;
    }
    else if(!priceLine->text().isEmpty()&&priceLine1->text().isEmpty()&&
            priceLine->text().isEmpty()&&!priceLine1->text().isEmpty()){
        QMessageBox::critical(0,QObject::tr("出错了"),"您的价格范围有误");
        return;
    }
    int x1,x2,y1,y2;
    x1=x2=y1=y2=0;
    ...
    while(count<len){
        ...
        if(tmp.compare("publish_year")==0){
            ...
            if(!flag){
                QMessageBox::critical(0,QObject::tr("出错了"),"未查到搜索的结果！");
                return;
            }
            if(str.length()==1){
...
/* 按照书的分类查询书籍 */
void database::searchBookByType(QTreeWidgetItem * tree)
{
    QSqlQuery query,query2;
    ...
    query.exec("select * from book "
               "where type = '"+text+"'");
    if(!query.isActive()){
        QMessageBox::critical(0,QObject::tr("出错了"),"未查询到结果！");
    }
...
bool database::createConnection()
{
    ...
    db.setPort(3306);
    if(!db.open()){
        QMessageBox::critical(0,QObject::tr("出错了"),db.lastError().text());
        return false;
    }
    return true;
}
...
void database::deleteBook()
{
    ...
    if(!query1.next()){
        QMessageBox::critical(NULL, "出错了", "该书号不存在",
                              QMessageBox::Yes);
        return;
    }
    ...
    if(!query.isActive()){
        QMessageBox::critical(NULL, "出错了", "删除失败",
                              QMessageBox::Yes);
    }
    else{
        QMessageBox::about(NULL,"恭喜您","删除成功");
    }
}
...
bool database::addBook()
{
    ...
    if(num>=99999){
        QMessageBox::critical(NULL,"出错了","数据库存储已满，请申请数据库扩容");
        return false;
    }
    tmp_num = ++num;
...
void database::oneAddBook()
{
    ...
    if(addBook()){
        QMessageBox::about(0,"恭喜您","添加成功！");
    }
    else{
        QMessageBox::critical(0,QObject::tr("出错了"),"添加失败");
    }
...
void database::multiAddBook()
{
    ...
    for(int i=0;i<count;i++){
        ...
        if(!addBook()){
            flag = false;
            QMessageBox::critical(NULL,"出错了","第" + QString::number(i+1) + "本书添加失败");
        }
    }
    if(flag){
        QMessageBox::about(NULL,"恭喜您","添加成功");
        //清空书本信息
        for(int i=0;i<7;i++){
            bookMessage[i].clear();
        }
...
void database::deleteUser()
{
    ...
    for(int i=0;i<max;i++){
        if(item0[i]->checkState() == Qt::Checked){
            ...
            if(tablewidget1->item(i,4)->text().toInt()>0){
                QMessageBox::critical(0,"出错了","该用户有未归还的书籍，不能删除！");
                return;
            }
            flag = true;
    ...
    if(!flag){
        QMessageBox::critical(0,"出错了","您尚未选择任何用户！");
        return;
    }
    if(query.isActive()){
        QMessageBox::about(0,"恭喜您","删除成功");
    }
    else{
        QMessageBox::critical(0,"出错了","删除失败");
    }
...
void database::grantLent()
{
    ...
    if(query.isActive()){
        if(isGrant){
            QMessageBox::about(0,"恭喜您","授权成功");
        }
        else{
            QMessageBox::about(0,"恭喜您","取消权限成功");
        }
    }
    else{
        if(isGrant){
            QMessageBox::critical(0,"出错了","授权失败");
        }
        else{
            QMessageBox::critical(0,"出错了","取消权限失败");
        }
        return;
    }
...
void database::returnBook()
{
    ...
    if(!flag){
        QMessageBox::critical(0,"出错了","您尚未选中任何书目！");
        return;
    }
    if(query.isActive()){
        QMessageBox::about(NULL,"恭喜您","归还成功！");
    }
    else{
        QMessageBox::about(NULL,"出错了","归还失败");
        return;
    }
    if(query1.isActive()){
        QMessageBox::about(NULL,"恭喜您","更新成功！");
    }
    else{
        QMessageBox::about(NULL,"出错了","更新失败");
        return;
    }
...
void database::lendBook()
{
    ...
    if(query.next()){
        ...
        if(!isLent){
            QMessageBox::critical(0,"出错了","您不具有借书权限,需等待管理员添加!");
            return;
        }
    }
    else{
    ...
    if(query.next()){
        ...
        if(totalNum>=3){
            QMessageBox::critical(0,"出错了","您的借书数目已达到限额（3本）！");
            return;
        }
    }
    else{
        ...
        if(item2[i]->checkState() == Qt::Checked){
            ...
            if(query.next()){
                ...
                if(num==0){
                    ...
                    QMessageBox::critical(NULL,"出错了",str);
                }
            }
            else{
                return;
    ...
    if(!flag){
        QMessageBox::critical(0,"出错了","您尚未选择任何书目！");
        return;
    }
    if(query1.isActive()){
        QMessageBox::about(0,"恭喜您","借阅成功");

    }
    else{
        QMessageBox::critical(0,"出错了","借阅失败");
        return;
    }
